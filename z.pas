uses graphABC;
var
  input: byte; m1, m2, p, u: real; n: integer;
...
var
  fo: real;
begin
  fo := (2) * power(x, 3) + (0) * power(x, 2) + (5) * x + (14);
  fun := fo;
end;
function fun1(var x: real): real;
begin
  var fo: real;
  fo := (power(x, 4) + 5 * power(x, 2)) / 2 + 14 * x;
  fun1 := fo;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        setpencolor(clgreen);
        u := p / 2 + x;
        Rectangle(x0 + round(s1 * mx), y0 - round(fun(u) * my), x0 + round(s1 * mx + l), y0);
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          u := p / 2 + x;
          if y0 - round(fun(u) * my) < y0 then
            Rectangle(x0 + round(s1 * mx + l), y0 - round(fun(u) * my), x0 + round(s1 * mx + (k + l)), y0);
        end;
      end;
      l := l + k;
      w := w + 1;
...
begin
  clearwindow;
  textout(0, 0, 'Вычисление площади фигуры, ограниченной кривой 2*x^3+(0)*x^2+5*x+14 и осью Ох (в положительной части по оси Оу)');
...
begin
  MaximizeWindow;
  Writeln('Вычисление площади фигуры, ограниченной кривой 2*x^3+(0)*x^2+5*x+14 и осью Ох (в положительной части по оси Оу)');
